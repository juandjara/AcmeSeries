...
<display:table name="series" id="row" requestURI="${requestURI}" 
	...
	<display:column title="${title}" sortable="false">
		<jstl:forEach var="tag" items="${row.tags}">
			<span class="label label-primary">
				...
				<jstl:if test="${requestURI=='series/producer/list.do'}">
					<a title="remove tag" 
						href="series/producer/removetag.do?multimediaId=${row.id}&tag=${tag}" 
						class="text-danger">
					<strong>X</strong>
					</a>
				</jstl:if>
	...
	<display:column title="${theader}">
		...
		<a href="episode/list.do?serieId=${row.id}"
		   class="btn btn-primary">
			<spring:message code="tvserie.episodes"/>
		</a>
		<a href="series/details.do?serieId=${row.id}"
		   class="btn btn-info">
			<spring:message code="acme.details"/>
		</a>
		<security:authorize access="hasRole('PRODUCER')">
		<jstl:if test="${requestURI=='series/producer/list.do'}">
			<a href="series/producer/edit.do?serieId=${row.id}" 
			   class="btn btn-primary">
...
<security:authorize access="hasRole('PRODUCER')">
	<acme:cancel url="series/producer/create.do" code="acme.create"/>
</security:authorize>